//
//  AttachmentHandlerUtility.swift
//  Expensiv
//
...
class AttachmentHandler: NSObject {
    ...
    func showAttachmentActionSheet() {
        ...
        actionSheet.addAction(UIAlertAction(title: Constants.cameraText, style: .default, handler: { (action) -> Void in
            self.authorisationStatus(attachmentTypeEnum: .camera, vc: self.currentVC!)
        }))
        actionSheet.addAction(UIAlertAction(title: Constants.phoneLibraryText, style: .default, handler: { (action) -> Void in
            self.authorisationStatus(attachmentTypeEnum: .photoLibrary, vc: self.currentVC!)
        }))
        actionSheet.addAction(UIAlertAction(title: Constants.cancelText, style: .cancel, handler: nil))
        // if iPhone
    ...
    func addAlertForSettings(_ attachmentTypeEnum: AttachmentType) {
        DispatchQueue.global(qos: .background).async {
            DispatchQueue.main.async {
                ...
                if attachmentTypeEnum == AttachmentType.camera {
                    alertTitle = Constants.alertForCameraAccessMessageText
                }
                if attachmentTypeEnum == AttachmentType.photoLibrary {
                    alertTitle = Constants.alertForPhotoLibraryMessageText
                }
                let cameraUnavailableAlertController = UIAlertController (title: alertTitle , message: nil, preferredStyle: .alert)
                let settingsAction = UIAlertAction(title: Constants.settingsText, style: .destructive) { (_) -> Void in
                    let settingsUrl = NSURL(string:UIApplication.openSettingsURLString)
                    if let url = settingsUrl {
                        UIApplication.shared.open(url as URL, options: [:], completionHandler: nil)
                    }
                }
                let cancelAction = UIAlertAction(title: Constants.cancelText, style: .default, handler: nil)
